...
def get_squeeze_score(daily_squeeze: Dict, weekly_squeeze: Dict = None) -> Dict:
    ...
    if weekly_squeeze:
        if weekly_squeeze.get('squeeze_fired') and weekly_squeeze.get('fire_direction') == 'RED':
            allow_entry = False  # GATE: weekly RED fire vetoes a fresh long (weekly-before-daily)
            score_adj -= 0.15
            reasons.append("Weekly squeeze fired RED — entry blocked (-0.15)")
        ...
        if weekly_squeeze.get('momentum', 0) < -5 and weekly_squeeze.get('momentum_accel', 0) < 0:
            allow_entry = False  # GATE: weekly momentum strongly negative + accelerating down
            reasons.append("Weekly momentum strongly negative — entry blocked")
...
def compute_volume_flow(df: pd.DataFrame, lookback: int = 20) -> Dict[str, float]:
    """
    Volume/flow confirmation from daily OHLCV — a proxy for the chart's
    Cum Delta / RelVol / Est-Sell read so the scanner can distinguish
    accumulation from distribution BEFORE a name reaches manual chart review.

      - relvol:        latest volume vs its `lookback` average (participation/conviction)
      - obv_slope:     normalized slope of On-Balance Volume (accumulation vs distribution)
      - updown_ratio:  up-volume / (up+down) volume over lookback (Est-Sell% inverse proxy)

    Returns a bounded flow_score in roughly [-1, 1] plus raw components.

    NOTE: this is a DAILY-BAR approximation. True intraday Cumulative Delta and
    Est. Sell % require tick/L2 data (ThinkorSwim). The chart remains the final
    adjudicator; this only pre-filters obvious distribution. Degrades to a neutral
    0.0 score when Volume is absent (e.g., backtest Close-only slices).
    """
    out = {"flow_score": 0.0, "relvol": 1.0, "obv_slope": 0.0, "updown_ratio": 0.5}
    try:
        ...
        # --- RelVol: latest bar volume vs trailing average ---
        relvol = float(vol.iloc[-1] / avg_vol)

        # --- OBV and its normalized, R^2-weighted slope (accumulation direction) ---
        ret_sign = np.sign(close.diff().fillna(0.0))
        obv = (ret_sign * vol).cumsum()
        obv_win = obv.iloc[-lookback:].values
        x = np.arange(len(obv_win))
        slope, _, r_value, _, _ = stats.linregress(x, obv_win)
        obv_slope = float(np.clip((slope / avg_vol) * (r_value ** 2), -1.0, 1.0))

        # --- Up/down volume balance over lookback (Est-Sell% proxy; 0.5 = balanced) ---
        chg = close.diff().iloc[-lookback:]
        v = vol.iloc[-lookback:]
        up_v = float(v[chg > 0].sum())
        dn_v = float(v[chg < 0].sum())
        tot = up_v + dn_v
        updown_ratio = float(up_v / tot) if tot > 0 else 0.5

        # --- Combine: direction (OBV slope + up/down skew) scaled by conviction (RelVol) ---
        direction = obv_slope + (updown_ratio - 0.5) * 2.0      # each component ~[-1, 1]
        direction = float(np.clip(direction / 2.0, -1.0, 1.0))
        conviction = min(float(np.clip(relvol, 0.0, 3.0)) / 1.5, 1.5)  # ~1.0 at avg vol
        flow_score = float(np.clip(direction * conviction, -1.0, 1.0))

        out.update({
            "flow_score": flow_score,
            "relvol": round(relvol, 2),
            "obv_slope": round(obv_slope, 4),
            "updown_ratio": round(updown_ratio, 3),
        })
        return out
    except Exception:
        return out
...
class StatisticalSignalModule:
    ...
    def compute_adaptive_weights(self, vol_regime: str) -> Dict[str, float]:
        """Compute dynamic weights based on volatility regime (incl. volume-flow confirmation)."""
        if vol_regime == "LOW_VOL":
            return {"momentum": 0.42, "mean_rev": 0.13, "trend": 0.27, "vol": 0.03, "flow": 0.15}
        elif vol_regime in ("HIGH_VOL", "EXTREME_VOL"):
            return {"momentum": 0.22, "mean_rev": 0.25, "trend": 0.33, "vol": 0.03, "flow": 0.17}
        return {"momentum": 0.35, "mean_rev": 0.17, "trend": 0.30, "vol": 0.03, "flow": 0.15}
    ...
    def compute_signals(
        ...
        def compute_single_signal(symbol: str, df: pd.DataFrame) -> Tuple[str, Dict]:
            ...
            # Volume/flow confirmation (accumulation vs distribution proxy).
            # Neutral 0.0 when Volume is unavailable (e.g. Close-only backtest slices).
            flow = compute_volume_flow(df, lookback=20)
            ...
            combined_score = (
                ...
                + weights.get("flow", 0.0) * flow_score
            )
            ...
            # RSI Divergence: computed for reference/display ONLY.
            # Per trading rule, divergence is NEVER factored into a directional read
            # (the tag has no determinable timing). Score contribution intentionally removed.
            divergence_data = _detect_rsi_divergence(prices)
            divergence_score = divergence_data["score"]
            # NOTE: do NOT add divergence_score to enhanced_score.
            ...
            try:
                ...
                enhanced_score += squeeze_score_adj  # RE-ENABLED: squeeze confirms / penalizes
            ...
            signal_dict = {
                ...
                "relvol": float(flow.get("relvol", 1.0)),
                "obv_slope": float(flow.get("obv_slope", 0.0)),
                "updown_ratio": float(flow.get("updown_ratio", 0.5)),
...
class ProfessionalTradingSystem:
    ...
    def _generate_recommendations(
        ...
        for sym in signals.keys():
            ...
            # WEEKLY SQUEEZE GATE: a weekly RED fire / strongly-negative weekly momentum
            # vetoes a fresh long (weekly-before-daily discipline). Downgrade to HOLD.
            if not sig.get("squeeze_allow_entry", True) and final_signal in ("BUY", "STRONG_BUY"):
                final_signal = "HOLD"
...
class BacktestEngine:
    ...
    def get_price_slice(self, date_):
        ...
        # Get the slice — carry FULL OHLCV when available so the volume-flow term
        # and the daily/weekly squeeze actually fire in backtest (they need
        # Volume/High/Low). Falls back to Close-only for symbols lacking OHLCV.
        # Uses .loc[:end_date] so there is no look-ahead beyond the analysis date.
        start_date = self.price_df.index[start_idx]
        end_date = date_
        price_slice = {}
        for sym in self.price_df.columns:
            full = self.all_price_data.get(sym) if isinstance(self.all_price_data, dict) else None
            if (isinstance(full, pd.DataFrame) and not full.empty
                    and {"Open", "High", "Low", "Close"}.issubset(set(full.columns))):
                try:
                    ohlcv = full.loc[:end_date].iloc[-(lookback_days + 1):]
                    if len(ohlcv) >= 20:
                        price_slice[sym] = ohlcv      # full OHLCV (+Volume if present)
                        continue
                except Exception:
                    pass
            # Fallback: Close-only (preserves prior behavior / no-OHLCV symbols)
